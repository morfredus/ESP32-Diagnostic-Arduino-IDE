/*
 * WEB_INTERFACE.H - Interface Web Dynamique v2.8.12
 */
...
String generateHTML() {
  ...
  html += ".status-message{min-height:24px;display:flex;align-items:center;gap:8px;}";
  html += ".status-live.status-message{justify-content:center;text-align:center;}";
  html += ".status-info{color:#0c5460;}";
  html += ".status-success{color:#155724;}";
  html += ".status-error{color:#721c24;}";
  ...
  html += ".toast-info{background:rgba(102,126,234,.95);}";
  html += ".toast-success{background:rgba(86,171,47,.95);}";
  html += ".toast-error{background:rgba(235,51,73,.95);}";
  html += ".gpio-hint{margin-top:15px;padding:12px;border-radius:10px;background:#eef2ff;color:#4c51bf;font-size:.9em;}";
...
String generateJavaScript() {
  ...
  js += "h+='<div class=\"info-item\"><div class=\"info-label\">Statut</div><div class=\"info-value status-message status-info\" id=\"builtin-led-status\">'+d.builtin.status+'</div></div>';";
  ...
  js += "h+='<div class=\"info-item\"><div class=\"info-label\">Statut</div><div class=\"info-value status-message status-info\" id=\"neopixel-status\">'+d.neopixel.status+'</div></div>';";
  js += "h+='<div class=\"info-item\" style=\"grid-column:1/-1;text-align:center\">';";
...
js += "function buildScreens(d){";
  ...
  js += "h+='<div class=\"info-item\"><div class=\"info-label\">Statut</div><div class=\"info-value status-message status-info\" id=\"oled-status\">'+d.oled.status+'</div></div>';";
  ...
  js += "h+='<div id=\"adc-status\" class=\"status-live status-message\">Cliquez pour tester</div></div>';";
  ...
  js += "h+='<div id=\"stress-status\" class=\"status-live status-message\">Non testé</div></div>';";
  ...
  js += "h+='<div id=\"gpio-status\" class=\"status-live status-message\">Cliquez pour tester</div>'";
  js += "+'<p class=\\"gpio-hint\\">ℹ️ Un GPIO indiqué comme ❌ FAIL peut simplement être réservé ou non câblé. Consultez la fiche technique avant de conclure à une défaillance.</p></div>';";
  ...
  js += "h+='<div id=\"wifi-status\" class=\"status-live status-message\">Cliquez pour scanner</div></div>';";
  ...
  js += "setStatusMessage('builtin-led-status','Test en cours...');";
  js += "try{";
  js += "const r=await fetch('/api/builtin-led-test');";
  js += "const d=await r.json();";
  js += "const msg=d.result||d.message||'Test terminé';";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('builtin-led-status',msg,type);";
  js += "}catch(e){setStatusMessage('builtin-led-status','Erreur: '+e,'error');}";
  js += "}";
  js += "async function ledBlink(){";
  js += "setStatusMessage('builtin-led-status','Clignotement en cours...');";
  js += "try{";
  js += "const r=await fetch('/api/builtin-led-control?action=blink');";
  js += "const d=await r.json();";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('builtin-led-status',d.message||'Clignotement terminé',type);";
  js += "}catch(e){setStatusMessage('builtin-led-status','Erreur: '+e,'error');}";
  js += "}";
  js += "async function ledFade(){";
  js += "setStatusMessage('builtin-led-status','Variation en cours...');";
  js += "try{";
  js += "const r=await fetch('/api/builtin-led-control?action=fade');";
  js += "const d=await r.json();";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('builtin-led-status',d.message||'Variation terminée',type);";
  js += "}catch(e){setStatusMessage('builtin-led-status','Erreur: '+e,'error');}";
  js += "}";
  js += "async function ledOff(){";
  js += "setStatusMessage('builtin-led-status','Extinction en cours...');";
  js += "try{";
  js += "const r=await fetch('/api/builtin-led-control?action=off');";
  js += "const d=await r.json();";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('builtin-led-status',d.message||'LED éteinte',type);";
  js += "}catch(e){setStatusMessage('builtin-led-status','Erreur: '+e,'error');}";
  js += "}";
  js += "async function ledOn(){";
  js += "setStatusMessage('builtin-led-status','Allumage en cours...');";
  js += "try{";
  js += "const r=await fetch('/api/builtin-led-control?action=on');";
  js += "const d=await r.json();";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('builtin-led-status',d.message||'LED allumée',type);";
  js += "}catch(e){setStatusMessage('builtin-led-status','Erreur: '+e,'error');}";
  js += "}";
  ...
  js += "setStatusMessage('oled-status','Test en cours (25s)...');";
  js += "try{";
  js += "const r=await fetch('/api/oled-test');";
  js += "const d=await r.json();";
  js += "const msg=d.result||d.message||'Test terminé';";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('oled-status',msg,type);";
  js += "}catch(e){setStatusMessage('oled-status','Erreur: '+e,'error');}";
  js += "}";
  js += "async function oledStep(step){";
  js += "setStatusMessage('oled-status','Étape en cours...');";
  js += "try{";
  js += "const r=await fetch('/api/oled-step?step='+step);";
  js += "const d=await r.json();";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('oled-status',d.message||'Étape exécutée',type);";
  js += "if(type==='error'){showToast(d.message||'Étape indisponible','error');}";
  js += "}catch(e){setStatusMessage('oled-status','Erreur: '+e,'error');}";
  js += "}";

  // FONCTIONS API - Affichage texte OLED
  js += "async function oledDisplayText(){";
  js += "const text=document.getElementById('oledText').value.trim();";
  js += "if(!text){setStatusMessage('oled-status','Veuillez entrer un message avant de lancer l\'affichage.','error');return;}";
  js += "setStatusMessage('oled-status','Affichage en cours...');";
  js += "try{";
  js += "const r=await fetch('/api/oled-message?message='+encodeURIComponent(text));";
  js += "const d=await r.json();";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('oled-status',d.message||'Message envoyé',type);";
  js += "}catch(e){setStatusMessage('oled-status','Erreur: '+e,'error');}";
  js += "}";
  js += "async function configOLED(){";
  js += "setStatusMessage('oled-status','Reconfiguration en cours...');";
  ...
  js += "const d=await r.json();";
  js += "const type=!r.ok||d.success===false?'error':(d.success?'success':'info');";
  js += "setStatusMessage('oled-status',d.message||'Configuration mise à jour',type);";
  js += "if(type==='success'){";
  js += "showToast(d.message||'Configuration OLED mise à jour','success');";
  js += "setTimeout(()=>location.reload(),800);";
  js += "}else if(type==='error'){";
  js += "showToast(d.message?'Erreur: '+d.message:'Erreur de configuration','error');";
  js += "}";
  js += "}catch(e){";
  js += "setStatusMessage('oled-status','Erreur: '+e,'error');";
  js += "showToast('Erreur: '+e,'error');";
  js += "}";
  js += "}";
  ...
  js += "function showUpdateIndicator(message='Mise à jour...'){";
  js += "const indicator=document.getElementById('updateIndicator');";
  js += "if(!indicator)return;";
  js += "indicator.textContent=message;";
  js += "indicator.classList.remove('toast-success','toast-error');";
  js += "indicator.classList.add('toast-info','show');";
  js += "}";
  js += "function hideUpdateIndicator(delay=500){";
  js += "const indicator=document.getElementById('updateIndicator');";
  js += "if(!indicator)return;";
  js += "setTimeout(()=>indicator.classList.remove('show'),delay);";
  js += "}";
  js += "function showToast(message,type='info',duration=2500){";
  js += "const indicator=document.getElementById('updateIndicator');";
  js += "if(!indicator)return;";
  js += "indicator.textContent=message;";
  js += "indicator.classList.remove('toast-info','toast-success','toast-error');";
  js += "const cls=type==='error'?'toast-error':(type==='success'?'toast-success':'toast-info');";
  js += "indicator.classList.add(cls,'show');";
  js += "setTimeout(()=>indicator.classList.remove('show'),duration);";
  js += "}";
  js += "function setStatusMessage(id,message,type='info'){";
  js += "const el=document.getElementById(id);";
  js += "if(!el)return;";
  js += "el.textContent=message;";
  js += "if(!el.classList.contains('status-live')){";
  js += "el.classList.remove('status-info','status-success','status-error');";
  js += "if(type==='error'){el.classList.add('status-error');}";
  js += "else if(type==='success'){el.classList.add('status-success');}";
  js += "else{el.classList.add('status-info');}";
  js += "}";
  js += "}";
  ...
  js += "}else{throw new Error(d.message||'Changement de langue refusé');}";
  js += "}).then(r=>r.json()).then(translations=>{";
  js += "updateInterfaceTexts(translations);";
  js += "showToast('Langue changée : '+lang.toUpperCase(),'success');";
  js += "}).catch(e=>{console.error(e);showToast('Erreur changement langue: '+e,'error');});";
  js += "}";
