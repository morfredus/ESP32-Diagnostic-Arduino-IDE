...
### Fixed
- Remplacement de la macro `WIFI_CONNECT_TIMEOUT_MS` par un unique `constexpr` dans `config.h`, supprimant les collisions de définition lors de la compilation Arduino.
- Documentation FR/EN (README, guides, références, interface web) synchronisée sur la v2.8.15 pour refléter cette consolidation.
...
### Fixed
- Correction de la compilation du firmware : la structure d'état WiFi est désormais définie avant utilisation, supprimant les erreurs `wifiRuntime` non déclaré.
- Documentation FR/EN mise à jour pour refléter la version 2.8.11 et ce correctif.
