...
- Assurez-vous d'exécuter le firmware 3.3.0 afin que `/api/set-language` conserve les succès explicites (FR/EN) et un HTTP 400 pour les codes non pris en charge tout en respectant le changelog de maintenance consolidé.
...
- Passez en version 3.3.0 (ou toute maintenance 3.x ultérieure) pour conserver l'intitulé « Non testé » et les traductions du bandeau sticky alignées sur la documentation bilingue mise à jour.
...
- Appelez `/api/memory-details` et analysez le pourcentage de `fragmentation` ainsi que les drapeaux PSRAM validés jusqu'en 3.3.0.
