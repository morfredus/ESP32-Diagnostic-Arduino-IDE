...
- Assurez-vous d'exécuter le firmware 3.5.1 afin que les liaisons de traduction en direct fournissent des succès explicites (FR/EN), un HTTP 400 sur les codes non pris en charge et des libellés harmonisés.
...
- Passez en version 3.5.1 (ou toute maintenance 3.x ultérieure) pour conserver l'intitulé « Non testé » et bénéficier du rafraîchissement instantané des traductions sur les onglets de diagnostics.
...
- Appelez `/api/memory-details` et analysez le pourcentage de `fragmentation` ainsi que les drapeaux PSRAM en vérifiant que les libellés traduits correspondent à la langue active en 3.5.1.
