...
- Ensure you are running firmware 3.3.11 so the unified JSON helpers back `/api/set-language` with explicit FR/EN successes, 400 responses for unsupported codes, and harmonised labels.
...
- Flash firmware 3.3.11 (or any later 3.x maintenance build) so the French catalogue keeps the "Non testé" placeholder and benefits from the restored translation-safe `String` handling.
...
- Call `/api/memory-details` and review the `fragmentation` percentage alongside the PSRAM flags produced by the unified JSON helpers in 3.3.11.
