...
- Ensure you are running firmware 3.5.0 so the refreshed live translation bindings back `/api/set-language` with explicit FR/EN successes, 400 responses for unsupported codes, and harmonised labels.
...
- Flash firmware 3.5.0 (or any later 3.x maintenance build) so the French catalogue keeps the "Non testé" placeholder and benefits from the instant translation refresh across diagnostics tabs.
...
- Call `/api/memory-details` and review the `fragmentation` percentage alongside the PSRAM flags while confirming the translated labels match the active language in 3.5.0.
