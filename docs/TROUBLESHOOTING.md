...
- Ensure you are running firmware 3.7.30 so the live translation bindings remain intact while the Wi-Fi splash and NimBLE safeguards are active.
...
- Flash firmware 3.7.30 (or any later 3.x maintenance build) so the French catalogue keeps the "Non testé" placeholder and benefits from the instant translation refresh across diagnostics tabs.
...
- Call `/api/memory-details` and review the `fragmentation` percentage alongside the PSRAM flags while confirming the translated labels match the active language in 3.7.30.
