...
2. Duplicate `exemple-config.h` and rename the copy to `config.h` (kept out of Git).
3. Edit `config.h` and provide at least one WiFi SSID/password pair. You may define fallback networks if desired.
...
| Web page shows stale WiFi data | Trigger “Actualiser l’onglet Sans fil” or reload; v2.8.8 auto-clears stale STA/AP metrics. |
| SoftAP only but WiFi dot stays red | Verify SSID/password in `config.h` or rely on SoftAP (orange dot indicates AP-only mode). |
...
- [`MODE_D_EMPLOI.fr.md`](MODE_D_EMPLOI.fr.md) – French edition of this setup guide.
